...
macro_rules! define_object_type {
    ($ty:ident, $name:expr, $check_id_type:expr) => {
        #[allow(dead_code)]
        pub struct $ty {
            handle: crate::handle::Handle,
        }

        impl crate::handle::FromID for $ty {
            #[inline]
            ...
            fn is_valid_id_type(id: ::libhdf5_sys::h5i::H5I_type_t) -> bool {
                ($check_id_type)(id)
            }
    ...
    ($ty:ident: $parent:ty, $name:expr, $check_id_type:expr) => {
        define_object_type!($ty, $name, $check_id_type);
        ...
        impl ::std::ops::Deref for $ty {
            ...
            fn deref(&self) -> &$parent {
                unsafe { &*(self as *const $ty as *const $parent) }
            }
        }
        ...
        impl ::std::ops::DerefMut for $ty {
            ...
            fn deref_mut(&mut self) -> &mut $parent {
                unsafe { &mut *(self as *mut $ty as *mut $parent) }
            }
        }
    };
}
